...
#if 0
namespace {
struct TestGlobalData : public ContextComponent {
    std::vector<int> data;
...
private:
    SPEMPE_CTXCOMP_TAG("test_TestGlobalData");
};

struct AlwaysGoingRightAvatar_VisibleState {
    int x = -1;
    HG_ENABLE_AUTOPACK(AlwaysGoingRightAvatar_VisibleState, x);
};

class AlwaysGoingRightAvatar : public SynchronizedObject<AlwaysGoingRightAvatar_VisibleState> {
public:
    AlwaysGoingRightAvatar(hg::QAO_RuntimeRef aRuntimeRef,
                           RegistryId aRegId,
                           SyncId aSyncId)
        : SyncObjSuper{aRuntimeRef, SPEMPE_TYPEID_SELF, 0, "AlwaysGoingRightAvatar", aRegId, aSyncId}
    {
        if (isMasterObject()) {
            _getCurrentState().x = 0;
        }
    }

    ~AlwaysGoingRightAvatar() override {
        if (isMasterObject()) {
            doSyncDestroy();
        }
    }
...
private:
    void _eventUpdate(IfMaster) {
        doSyncUpdate();
        _getCurrentState().x += 1;
    }

    void _eventUpdate(IfDummy) {
        SPEMPE_SYNCOBJ_BEGIN_EVENT_UPDATE_OVERRIDE();
        ccomp<TestGlobalData>().putData(_getCurrentState().x);
    }
...
SPEMPE_GENERATE_DEFAULT_SYNC_HANDLERS(AlwaysGoingRightAvatar, (CREATE, UPDATE, DESTROY));

void AlwaysGoingRightAvatar::_syncCreateImpl(SyncDetails& aSyncDetails) const {
    std::cout << "AlwaysGoingRightAvatar::_syncCreateImpl called!" << std::endl;
    SPEMPE_SYNC_CREATE_DEFAULT_IMPL(AlwaysGoingRightAvatar, aSyncDetails);
}

void AlwaysGoingRightAvatar::_syncUpdateImpl(SyncDetails& aSyncDetails) const {
    SPEMPE_SYNC_UPDATE_DEFAULT_IMPL(AlwaysGoingRightAvatar, aSyncDetails);
}

void AlwaysGoingRightAvatar::_syncDestroyImpl(SyncDetails& aSyncDetails) const {
    SPEMPE_SYNC_DESTROY_DEFAULT_IMPL(AlwaysGoingRightAvatar, aSyncDetails);
}
} // namespace

TEST_F(SPeMPE_SynchronizedTest, NetworkingManagerTest) {
    // Set up server context:
    std::uint16_t serverPort;
    {
        _serverCtx->setToMode(GameContext::Mode::Server);

        auto netwMgr = hg::QAO_UPCreate<NetworkingManagerOne>(_serverCtx->getQAORuntime().nonOwning(),
                                                              0, /* exec prio*/
                                                              2  /* buffering len*/ );
        netwMgr->setToMode(NetworkingManagerInterface::Mode::Server);
        netwMgr->getServer().start(0);
        serverPort = netwMgr->getServer().getLocalPort();

        _serverCtx->attachAndOwnComponent(std::move(netwMgr));

        hg::QAO_PCreate<AlwaysGoingRightAvatar>(_serverCtx->getQAORuntime(),
                                                _serverCtx->getComponent<MNetworking>().getRegistryId(),
                                                SYNC_ID_NEW);
    }
    ...
    {
        _serverCtx->setToMode(GameContext::Mode::Client);

        _clientCtx->attachAndOwnComponent(std::make_unique<TestGlobalData>());

        auto netwMgr = hg::QAO_UPCreate<NetworkingManagerOne>(_clientCtx->getQAORuntime().nonOwning(),
                                                              0, /* exec prio*/
                                                              2  /* buffering len*/ );
        netwMgr->setToMode(NetworkingManagerInterface::Mode::Client);
        netwMgr->getClient().connect(0, sf::IpAddress::LocalHost, serverPort);

        _clientCtx->attachAndOwnComponent(std::move(netwMgr));
    }
    ...
    // Run contexts:
    {
        ...
        for (int i = 0; i < 150; i += 1) {
            ...
            //if (_clientCtx->getComponent<TestGlobalData>().data.size() == 100) {
            //    break;
            //}
        }
    ...
    // Check results:
    {
        auto& results = _clientCtx->getComponent<TestGlobalData>().data;
