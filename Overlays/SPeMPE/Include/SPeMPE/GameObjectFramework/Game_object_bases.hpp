...
#define USPEMPE_GENERATE_DEFAULT_SYNC_HANDLER_UPDATE_AD(_class_name_) \
    USPEMPE_GENERATE_DEFAULT_SYNC_HANDLER_UPDATE(_class_name_)
...
//! Calls the default sync/UPDATE_AD implementation of a SynchronizedObject
//! generated by SPEMPE_GENERATE_DEFAULT_SYNC_HANDLERS.
#define SPEMPE_SYNC_UPDATE_AD_DEFAULT_IMPL(_class_name_, _sync_details_) \
    do { \
        const auto __spempeimpl_oldPackMode = _getCurrentState().getPackMode(); \
        /* TODO: Align sync details / cmp / pack mode */ \
        Compose_USPEMPE_Update##_class_name_((_sync_details_).getNode(), \
                                             (_sync_details_).getRecepients(), \
                                             getSyncId(), \
                                             (_sync_details_).getFlags(), \
                                             _getCurrentState()) \
        _getCurrentState().setPackMode(__spempeimpl_oldPackMode); /* TODO problem with constness */ \
    } while (false)
