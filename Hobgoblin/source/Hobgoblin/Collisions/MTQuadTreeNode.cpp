...
#define Self MTQuadTreeNode

namespace col {
    
    MTQuadTreeNode::MTQuadTreeNode(double x, double y, double w, double h, size_t maxdep, size_t maxobj, MTQuadTreeDomain * domain)
        : bbox(x, y, w, h, 0)
        , parent(nullptr)
        , MAX_DEPTH(maxdep)
        , MAX_OBJECTS(maxobj)
    ...
    MTQuadTreeNode::MTQuadTreeNode(double x, double y, double w, double h, MTQuadTreeNode * parent, size_t maxdep, size_t maxobj, MTQuadTreeDomain * domain)
        : bbox(x, y, w, h, 0)
        , parent(parent)
        , MAX_DEPTH(maxdep)
        , MAX_OBJECTS(maxobj)
    ...
    void MTQuadTreeNode::split(bool fit) {
        ...
        if (chl[0] == nullptr) {
            ...
            chl[0].reset(new MTQuadTreeNode(x, y, w / 2, h / 2, this, MAX_DEPTH, MAX_OBJECTS, domain));                 //Top-left
            chl[1].reset(new MTQuadTreeNode(x + w / 2, y, w / 2, h / 2, this, MAX_DEPTH, MAX_OBJECTS, domain));         //Top-right
            chl[2].reset(new MTQuadTreeNode(x, y + h / 2, w / 2, h / 2, this, MAX_DEPTH, MAX_OBJECTS, domain));         //Bottom-left
            chl[3].reset(new MTQuadTreeNode(x + w / 2, y + h / 2, w / 2, h / 2, this, MAX_DEPTH, MAX_OBJECTS, domain)); //Bottom-right
    ...
    bool MTQuadTreeNode::obj_fits(const QuadTreeEntity & entity) const {

        return (entity.bbox).enveloped_by(bbox);

        }

    void MTQuadTreeNode::obj_add(QuadTreeEntity & entity) {

        objects.push_back(entity.ptr.index());

        counter += 1;

        entity.holder.multithr = this;
        entity.type = QuadTreeEntity::MULTITHR;
    ...
    void MTQuadTreeNode::obj_add_fit(QuadTreeEntity & entity) {

        MTQuadTreeNode * curr = this;
    ...
    void MTQuadTreeNode::obj_list_erase(std::list<size_t>::iterator iter) {

        objects.erase(iter);

        }

    int  MTQuadTreeNode::obj_new_holder(const QuadTreeEntity & entity) const {
    ...
    void MTQuadTreeNode::visit(PairsContainer & pairs_deq) const {
    ...
    void MTQuadTreeNode::visit_with(PairsContainer & pairs_deq,
    ...
    void MTQuadTreeNode::migrate_entity(QuadTreeEntity & entity, MTQuadTreeNode * new_holder) {
        
        new_holder->objects.splice(new_holder->objects.end(), objects, entity.my_iter);

        entity.holder.multithr = new_holder;
        entity.type = QuadTreeEntity::MULTITHR;
    ...
    void MTQuadTreeNode::draw(sf::RenderTarget * rt) {
        ...
        /*sf::Text tex(std::to_string(counter), *def_font, 16);
        tex.setFillColor(sf::Color::White);
        tex.setPosition(bbox.x + 10, bbox.y + 10 + 18 * depth);
        rt->draw(tex);*/
