...
struct ContextSettings {
    ...
    explicit ContextSettings(
        PZInteger aDepth = 0,
        PZInteger aStencil = 0,
        PZInteger aAntialiasing = 0,
        PZInteger aMajor = 1,
        PZInteger aMinor = 1,
        Attribute aAttributes = Attribute::Default,
        bool      aSRgb = false
    )
        : depthBits(aDepth)
        , stencilBits(aStencil)
        , antialiasingLevel(aAntialiasing)
        , majorVersion(aMajor)
        , minorVersion(aMinor)
        , attributeFlags(aAttributes)
        , sRgbCapable(aSRgb)
    {
    }

    PZInteger depthBits;         //!< Bits of the depth buffer
    PZInteger stencilBits;       //!< Bits of the stencil buffer
    PZInteger antialiasingLevel; //!< Level of antialiasing
    PZInteger majorVersion;      //!< Major number of the context version to create
    PZInteger minorVersion;      //!< Minor number of the context version to create
    Attribute attributeFlags;    //!< The attribute flags to create the context with
    bool      sRgbCapable;       //!< Whether the context framebuffer is sRGB capable
};

inline
ContextSettings::Attribute operator|(ContextSettings::Attribute aLhs, ContextSettings::Attribute aRhs) {
    return static_cast<ContextSettings::Attribute>(
